...
(function () {
  ...
  var TERMS = {
    ...
    '.mcp.json':       { id: 'g-mcp-json',       desc: 'Claude Code가 어떤 외부 도구(MCP 서버)에 연결할지 적어 두는 설정 파일. 안의 env 항목에 Access Token을 넣는다. 비밀 값을 담는 .env와는 다른 파일.' },
    'env':             { id: 'g-mcp-json',       desc: '.mcp.json 안에 든 칸으로, 연결에 필요한 값(토큰)을 적는 곳. 비밀 값 파일 .env와는 무관.' },
    'npx':             { id: 'g-npx',            desc: '설치 없이 필요한 프로그램을 그때그때 내려받아 바로 실행해 주는 도구(Node.js에 포함).' },
    'npx -y @supabase/mcp-server-supabase@latest': { id: 'g-npx', desc: 'Supabase 연결 프로그램(MCP 서버)을 그때그때 내려받아 실행하라는 명령. 실행은 npx가 맡음.' },
    'sbp_':            { id: 'g-access-token',   desc: 'Supabase Access Token이 시작하는 접두어. 계정 전체를 제어할 만큼 강력해 파일에 직접 넣고 채팅엔 안 넣음.' },
    ...
    'supabase':        { id: 'g-supabase',      desc: '대화·문서 데이터를 저장하는 무료 클라우드 데이터베이스(백엔드) 서비스.' },
    'project url':     { id: 'g-supabase-url',  desc: 'Supabase 프로젝트의 접속 주소(환경변수 SUPABASE_URL).' },
    'supabase_url':    { id: 'g-supabase-url',  desc: 'Supabase 프로젝트의 접속 주소(환경변수 이름).' },
    'anon public':     { id: 'g-supabase-anon', desc: 'Supabase 접속에 쓰는 공개(anon) 키.' },
    'supabase_anon_key': { id: 'g-supabase-anon', desc: 'Supabase 접속용 공개(anon) 키(환경변수 이름).' },
    'fetch':           { id: 'g-fetch',         desc: '인터넷으로 데이터를 주고받는(요청·응답) 자바스크립트 기본 기능. 설치 없이 바로 씀.' },
    'sessions-ref.sql': { id: 'g-sessions-sql', desc: '대화 세션을 저장할 Supabase 테이블을 만드는 SQL 파일.' },
    ...
    'jwt secret':      { id: 'g-jwt',            desc: '로그인한 사람의 출입증(토큰 JWT)이 진짜인지 확인·서명하는 열쇠. (이 과정에서는 다루지 않음)' },
    'jwt':             { id: 'g-jwt',            desc: '로그인한 사람에게 발급되는 디지털 출입증(토큰).' },
    'db password':     { id: 'g-db-password',    desc: '데이터베이스에 직접 로그인하는 비밀번호. 매우 강력해 노출 금지.' },
    'connection string': { id: 'g-db-password', desc: '데이터베이스에 직접 접속하는 주소·비밀번호를 합친 정보. 노출 금지.' },
